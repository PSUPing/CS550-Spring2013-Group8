...
class Number( Expr ) :
	...
	def __init__( self, v=0 ) :
		self.value=P2funcs.Value(v,False)
...
class ListType( Expr ) :
	...
	def eval( self, nt, ft ) :
		self.listVals = P2funcs.Value(-1,True)
		for i in self.inp:
			if type(i) is int:
				i=P2funcs.Value(i,False)
			self.listVals=memory.cons(i,self.listVals,nt)
...
class Concat( Expr ) :
	...
	def eval( self, nt, ft ) :
		...
		return memory.concat(self.lhs.eval(nt,ft),self.rhs.eval(nt,ft),nt)
...
class FunCall( Expr ) :
	...
	def eval( self, nt, ft ) :
		...
		if self.name in self.builtins : 
			...
			if len(self.argList) > 0 :
				param1 = self.argList[0].eval( nt, ft )
			
			if len(self.argList) > 1 :
				param2 = self.argList[1].eval( nt, ft )

			# Adds param1 (int / list) to the list in param2
			if self.name == 'cons' :
				#if type(param2) is list :
				return memory.cons(param1, param2,nt)
				'''else :
